...
func main() {
	...
	fmt.Println("Callout to delete user follows callout")

	userResponse, err := client.DeleteUserFollows(context.Background(), *userID, *targetUserID)
	if err != nil {
		log.Panicf("user delete follows error: %v", err)
	}
