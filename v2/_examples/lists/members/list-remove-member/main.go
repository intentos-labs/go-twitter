...
func main() {
	...
	fmt.Println("Callout to list remove member callout")

	listResponse, err := client.RemoveListMember(context.Background(), *listID, *userID)
	if err != nil {
		log.Panicf("list add remove error: %v", err)
	}
