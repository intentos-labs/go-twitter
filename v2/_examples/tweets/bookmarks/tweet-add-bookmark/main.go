...
func main() {
	...
	fmt.Println("Callout to add tweet bookmark callout")

	tweetResponse, err := client.AddTweetBookmark(context.Background(), *userID, *tweetID)
	if err != nil {
		log.Panicf("add tweet bookmarks error: %v", err)
	}
