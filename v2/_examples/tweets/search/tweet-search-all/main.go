...
*
	...
	In order to run, the user will need to provide the bearer token and query.
...
func main() {
	...
	fmt.Println("Callout to tweet search callout")

	tweetResponse, err := client.TweetSearch(context.Background(), *query, opts)
	if err != nil {
		log.Panicf("tweet search error: %v", err)
	}
